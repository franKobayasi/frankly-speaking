...
我想如果有一個 Agent，可以自動去查 Log、自动连线 DB 撈資料、自动追踪程式码版本，最后给我一份 Root Cause 分析报告，那該有多好？於是 OptiSentry 就誕生了。
...
OptiSentry 使用以下技術：Agent Framework 採用 Claude Code（使用 Pi agent 架構）；Deployment 使用 GCP Cloud Run / GKE / Cloud Function；Database 支援 MongoDB / PostgreSQL / MSSQL；Log 查詢使用 Cloud Logging；Incident Management 整合 Azure DevOps（自動建維運單）；Notification 透過 Slack 通知。

## 實際使用情境

假設今天 Ad Management 服務的回應時間變長，你只需要對 Agent 說：「帮我查一下 ad-management 服务最近的错误」

Agent 就會自動：讀取 ad-management 的服務定義、查詢 Cloud Logging 的 ERROR logs、（如果有設定）連線 MongoDB 查詢相關資料、取得 Cloud Run 的部署版本、用 git worktree 切到對應 commit 分析程式碼、產出 Root Cause 分析報告。

整個過程不需要你手動切換任何畫面。

## 總結

OptiSentry 是我用 Claude Code 打造的第一個維運輔助工具，目標是讓 SRE 能更有效率地調查生產問題。目前它已經可以自動完成大部分的調查工作，未來我還計畫加入 Sentry 錯誤報告自動串接、自動生成維運報告、更多資料庫支援，以及 MCP 對外暴露讓其他工具也能使用。
