...
這是一個基於 **Claude Code** 的 維運 維運輔助平台，包含兩個主要的 Agent：

**investigate** 是 維運 調查員 — 自主蒐集 Log / DB / Code 證據，分析 Root Cause。當生產環境出問題時，只要告訴它「我們的 XXX 服務好像掛了」，它就會自動完成調查，給你一份完整的分析報告。
...
這是我用 Claude Code 打造的第一個維運輔助工具，目標是讓 維運 能更有效率地調查生產問題。目前它已經可以自動完成大部分的調查工作，未來還計畫加入 Sentry 錯誤報告自動串接、自動生成維運報告、更多資料庫支援，以及 MCP 對外暴露讓其他工具也能使用。
